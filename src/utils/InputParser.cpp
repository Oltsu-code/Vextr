...
void InputParser::flushEscape() { emit(Key::Escape); }

void InputParser::feed(unsigned char byte) {
  buf.push_back(byte);
  if (buf[0] == 13)
    buf[0] = 10; // this bc windows....
  tryParse();
}
...
void InputParser::tryParse() {
  ...
  if (buf[1] == '[') {
    ...
    if (buf.size() == 6 && buf[2] == '1' && buf[3] == ';') {
      ...
      switch (buf[5]) {
      case 'A':
        emit(Key::Up, shift, ctrl, alt);
        return;
      case 'B':
        emit(Key::Down, shift, ctrl, alt);
        return;
      case 'C':
        emit(Key::Right, shift, ctrl, alt);
        return;
      case 'D':
